...
// // Standard bracket seeding order helper (kept for potential future formats)
// function generateSeedOrder(size: number): number[] {
//     if (size & (size - 1)) throw new Error('size must be power of two');
//     const build = (n: number): number[] => {
//         if (n === 1) return [1];
//         const prev = build(n / 2);
//         const mirror = prev.map(s => n + 1 - s);
//         const out: number[] = [];
//         for (let i = 0; i < prev.length; i++) out.push(prev[i], mirror[i]);
//         return out;
//     };
//     return build(size);
// }
...
// // Remove BYE-involved matches from the bracket after auto-advancing winners
// function collapseByeMatches(): void {
//     // Intentionally minimal: keep single-player matches visible as requested.
//     // Remove only matches where BOTH participants are missing.
//     if (!state) return;
//     const kept: TournamentMatch[] = [];
//     for (const m of state.matches) {
//         const hasAny = !!m.p1 || !!m.p2;
//         if (hasAny) kept.push(m);
//     }
//     state.matches = kept;
// }
