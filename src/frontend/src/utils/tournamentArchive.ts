import { Tournament, TournamentPlayer, TournamentMatch } from '../../../shared/tournamentTypes';
...
async function populateArchiveList(): Promise<void> {
	...
	try {
		...
		if (local && local.length) {
			// Build friendly sequential numbers by createdAt (oldest = #1)
			...
			list = local.map(a => ({
				...
				championAlias: a.champion?.alias || null,
				_source: 'local'
			}));
		}
	} catch {}
	if (!list.length) {
		const remote = await fetchTournamentList();
		list = remote.map((t: any) => ({
			// Keep internal id for API lookups, use displayId for showing
	...
	listEl.querySelectorAll<HTMLButtonElement>('button[data-id]').forEach(btn => {
		btn.addEventListener('click', async () => {
			...
			currentDisplayId = btn.dataset.displayId ? Number(btn.dataset.displayId) : undefined;
...
async function showTournamentDetail(id: number): Promise<void> {
	...
	const playersHtml = data.players.map((p: TournamentPlayer) => {
		...
		return `<li>${p.alias} ${p.eliminated ? '✖' : '✓'} (${stats})</li>`;
	}).join('');
	const historyHtml = data.matchHistory.length
	? data.matchHistory.map((h: TournamentMatch) => {
		const p1 = h.p1 || '?';
		const p2 = h.p2 || '?';
		const w = h.winner || '?';
	...
	: '<li>(no matches)</li>';
	const champName = data.champion?.alias;

	detailEl.innerHTML = 
		`<div class="t-archive-detail-inner">
			<h4>Tournament #${currentDisplayId ?? data.tournamentId}</h4>
...
async function showLocalTournamentDetail(id: number): Promise<void> {
	...
	const playersHtml = (data.players || []).map((p: TournamentPlayer) => {
		return `<li>${p.alias}</li>`;
	}).join('');
	const historyHtml = (data.matches || []).length
		? data.matches.map((m: any) => {
			const p1 = m.p1?.alias || '—';
			const p2 = m.p2?.alias || '—';
			const w = m.winner?.alias || '—';
			return `<li>#${m.id} ${p1} vs ${p2} → <strong>${w}</strong></li>`;
		}).join('')
		: '<li>(no matches)</li>';
	const champName = data.champion?.alias;
