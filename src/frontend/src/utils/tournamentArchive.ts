...
// Local archive access (legacy prototype compatibility)
...
function getLocalArchive(): LocalArchiveEntry[] {
  try {
    const raw = localStorage.getItem('tournamentArchive');
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    if (Array.isArray(parsed)) return parsed as LocalArchiveEntry[];
    return [];
  } catch { return []; }
}
...
async function populateArchiveList(): Promise<void> {
  ...
  const local = getLocalArchive();
    if (local && local.length) {
      ...
      list = local.map((a: LocalArchiveEntry) => ({
        tournamentId: a.tId,
...
async function showTournamentDetail(id: number): Promise<void> {
  ...
  if (!data) { tDetailArchive.innerHTML = `<p class="t-archive-error">Failed to load tournament.</p>`; return; }
...
async function showLocalTournamentDetail(id: number): Promise<void> {
  ...
  const list = getLocalArchive();
  const data = list.find((a: any) => a.tId === id);
  if (!data) { detailEl.innerHTML = `<p class="t-archive-error">Tournament not found in local archive.</p>`; return; }
